...
def run_pipeline_2(file_name):
    ...
    # read the files
    df = pd.read_json(open(f'artifact_for_rq1/{file_name}.json', 'r'))
    ...
    with open(f'results_for_rq1/results_{file_name}_exp2.json', 'a') as write_file:
        result_dict = {}
        for i, (index, row) in enumerate(df.iterrows(), starting_point):
            ...
            result_dict['PromptForExperiment'] = row['PromptForExperiment2'][0]
            result_dict['RespForExperiment'] = row['RespForExperiment']

            print('evaluating row: ', i, index)

            resp = gpt_embeding_automation(row["normalized_models"], row['ObjectForExp2'], row["PromptForExperiment2"][0])
            result_dict['SecondEmbedResp'] = resp
...
if __name__ == "__main__":
    ...
    # # commit sharing pipeline
    run_pipeline_2('sample2')
